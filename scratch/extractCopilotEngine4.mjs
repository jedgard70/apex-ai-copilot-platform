...
for (const fnName of functionsToExtract) {
  // Using single backslash since it's a string literal that evaluates to \s, \S, etc.
  // Wait, in a string literal, '\s' evaluates to 's'. So we DO need '\\s'.
  // My previous was '\\\\s' which evaluates to '\\s'. And RegExp('\\s') means match whitespace.
  // RegExp constructor takes '\\s' to mean '\s'.
  // So double backslash is correct: '\\s'.
  const functionRegex = new RegExp('function\\\\s+' + fnName + '\\\\s*\\\\([^)]*\\\\)\\\\s*(?::\\\\s*[^\\\\{]+)?\\\\s*\\\\{');
  
  // Actually let's just use a simpler regex!
  const simplerRegex = new RegExp(\`function\\\\s+\${fnName}\\\\s*\\\\([^\\\\)]*\\\\)\\\\s*(?::\\\\s*[^\\\\{]+)?\\\\s*\\\\{\`);
  
  // Even safer: use string methods!
  const searchString = \`function \${fnName}(\`;
  let idx = mainContent.indexOf(searchString);
  if (idx === -1) {
    console.warn("Function " + fnName + " not found with standard spacing, trying regex fallback");
    const fbRegex = new RegExp('function\\\\s+' + fnName + '\\\\s*\\\\(');
    const m = fbRegex.exec(mainContent);
    if (m) idx = m.index;
  }
  ...
  if (idx !== -1) {
    ...
    for (; i < mainContent.length; i++) {
      ...
      if (started && braceCount === 0) {
        const fullFunction = mainContent.substring(idx, i + 1);
        newlyExtracted.push('export ' + fullFunction);
        
        mainContent = mainContent.substring(0, idx) + mainContent.substring(i + 1);
        break;
      }
    }
  } else {
    console.warn("Function " + fnName + " completely not found");
  }
}
...
console.log('CopilotEngine extracted and updated v4');
