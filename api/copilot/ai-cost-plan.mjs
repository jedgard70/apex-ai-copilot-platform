...
export default async function handler(req, res) {
  if (req.method !== 'POST') {
    ...
    return sendJson(res, 405, { error: 'Method not allowed', providerStatus: 'estimated-local' })
  }
  try {
    const body = req.body && typeof req.body === 'object' ? req.body : {}
    return sendJson(res, 200, { plan: createAiCostPlan(String(body.goal || '')) })
  } catch (error) {
    return sendJson(res, 500, { error: error?.message || 'ai_cost_plan_failed', providerStatus: 'estimated-local' })
  }
}
