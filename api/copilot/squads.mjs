import { createSquad, getSquads, getSquadById, runSquadStep, approveCheckpoint } from '../../server/service/apexSquads.mjs';
...
export default async function handler(req, res) {
  try {
    ...
    if (method === 'GET' && !url.includes('/run') && !url.includes('/approve')) {
      const idMatch = url.match(/\/squads\/(sqd_\d+)/);
      if (idMatch) {
    ...
    if (method === 'POST' && url.includes('/run')) {
      const idMatch = url.match(/\/squads\/(sqd_\d+)\/run/);
    ...
    if (method === 'POST' && url.includes('/approve')) {
      const idMatch = url.match(/\/squads\/(sqd_\d+)\/approve/);
      if (!idMatch) return sendJson(res, 400, { error: 'ID inválido' });
