 ...
 */
export async function detectProblems(options = {}) {
  const mode = options.mode === 'quick' ? 'quick' : 'full'
  const deepScan = mode === 'full'
  const problems = []
  ...
  // 2..N diagnostics that can be expensive
  if (deepScan) {
    // 2. TypeScript errors
    try {
  ...
  // 7. Outdated dependencies (deep scan only)
  if (deepScan) {
    try {
...
export async function getAutoFixStatus(options = {}) {
  const deep = Boolean(options.deep)
  const problems = await detectProblems({ mode: deep ? 'full' : 'quick' })
  return {
    healthy: problems.length === 0,
    problemCount: problems.length,
    autoFixableCount: problems.filter(p => p.autoFixable).length,
    problems,
    scanMode: deep ? 'full' : 'quick',
