...
import { logUsage } from '../analytics/costTracker.mjs'
...
const providerOrder = [
  { name: "apex-runtime", label: "Apex Runtime (Local)" },
...
export async function getProviderChain(options = {}) {
  ...
  // 1. Injeta o Apex Runtime local como motor PRIMÁRIO / Fallback Local Vault
  ...
  // 2. Gemini — API nativa com X-goog-api-key
  if (geminiKey) {
  ...
  if (openAiKey) {
    chain.push({
      ...
      apiKey: openAiKey,
      model: preferredModel && preferredModel.startsWith("gpt") ? preferredModel : "gpt-4o",
      label: "OpenAI",
  ...
  if (groqKey) {
    chain.push({
      ...
      apiKey: groqKey,
      model: preferredModel && preferredModel.includes("groq") ? preferredModel : "llama3-70b-8192",
      label: "Groq",
  ...
  if (anthropicKey) {
    chain.push({
      ...
      apiKey: anthropicKey,
      model: preferredModel && preferredModel.startsWith("claude") ? preferredModel : "claude-3-5-sonnet-20240620",
  ...
  if (hfKey) {
    chain.push({
      ...
      apiKey: hfKey,
      model: preferredModel && preferredModel.includes("/") ? preferredModel : "Qwen/Qwen2.5-72B-Instruct",
      label: "Hugging Face",
      models: ["Qwen/Qwen2.5-72B-Instruct", "meta-llama/Meta-Llama-3-8B-Instruct", "mistralai/Mistral-Nemo-Instruct-2407"]
    })
  }
    ...
    for (const model of modelsToTry) {
      ...
      try {
        ...
        if (provider.name === "apex-runtime") {
          ...
          if (response.ok) {
            ...
            if (geminiData.usageMetadata) {
              logUsage(provider.name, model, geminiData.usageMetadata)
            }
        ...
        if (response.ok) {
          ...
          // Log tokens for cost tracking
          if (data.usage) {
            logUsage(provider.name, data.model || model, data.usage)
          }
