...
import { recordRateLimit } from '../../service/rateLimitMonitor.mjs'
...
async function checkOpenAI() {
  ...
  if (!key) return { id: 'openai', name: 'OpenAI (GPT-4o + DALL-E 3)', status: 'unconfigured', message: 'OPENAI_API_KEY não configurado.', topUpUrl: 'https://platform.openai.com/account/billing' }
  try {
    ...
    if (!res.ok) {
      ...
      return { id: 'openai', name: 'OpenAI (GPT-4o + DALL-E 3)', status: 'error', message: err?.error?.message || `Status ${res.status}`, topUpUrl: 'https://platform.openai.com/account/billing' }
    }
    // Try to get credits from usage API
    const usageRes = await safeFetch('https://api.openai.com/v1/dashboard/billing/subscription', {
      headers: { Authorization: `Bearer ${key}` }
    }, 5000).catch(() => null)
    let creditNote = ''
    if (usageRes?.ok) {
      const u = await usageRes.json().catch(() => ({}))
      if (u.hard_limit_usd) creditNote = ` | Limite: $${u.hard_limit_usd}`
    }
    return { id: 'openai', name: 'OpenAI (GPT-4o + DALL-E 3)', status: 'ok', message: `Chave válida. GPT-4o + DALL-E 3 disponíveis.${creditNote}`, capabilities: ['Chat', 'Image Generation', 'Embeddings'], topUpUrl: 'https://platform.openai.com/account/billing' }
  } catch (err) {
    return { id: 'openai', name: 'OpenAI (GPT-4o + DALL-E 3)', status: 'error', message: `Erro: ${scrub(err?.message)}`, topUpUrl: 'https://platform.openai.com/account/billing' }
  }
}
...
async function checkAnthropic() {
  ...
  if (!key) return { id: 'anthropic', name: 'Anthropic / Claude Sonnet', status: 'unconfigured', message: 'ANTHROPIC_API_KEY não configurado.', topUpUrl: 'https://console.anthropic.com/settings/billing' }
  try {
    const res = await safeFetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'x-api-key': key, 'anthropic-version': '2023-06-01', 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: 'claude-3-haiku-20240307', max_tokens: 5, messages: [{ role: 'user', content: 'OK' }] })
    }, 10000)
    const data = await res.json().catch(() => ({}))
    if (!res.ok) return { id: 'anthropic', name: 'Anthropic / Claude Sonnet', status: 'error', message: data?.error?.message || `Status ${res.status}`, topUpUrl: 'https://console.anthropic.com/settings/billing' }
    return { id: 'anthropic', name: 'Anthropic / Claude Sonnet', status: 'ok', message: 'Chave válida. Claude Haiku/Sonnet disponíveis.', capabilities: ['Chat', 'Analysis'], topUpUrl: 'https://console.anthropic.com/settings/billing' }
  } catch (err) {
    return { id: 'anthropic', name: 'Anthropic / Claude Sonnet', status: 'error', message: `Erro: ${scrub(err?.message)}`, topUpUrl: 'https://console.anthropic.com/settings/billing' }
  }
}

// ─── Groq (LLM Ultra-rápido) ────────────────────────────────────────────────
async function checkGroq() {
  const key = process.env.GROQ_API_KEY
  if (!key) return { id: 'groq', name: 'Groq (LLM Ultra-rápido)', status: 'unconfigured', message: 'GROQ_API_KEY não configurado.', topUpUrl: 'https://console.groq.com' }
  try {
    const t0 = Date.now()
    ...
    if (!res.ok) {
      const data = await res.json().catch(() => ({}))
      return { id: 'groq', name: 'Groq (LLM Ultra-rápido)', status: res.status === 429 ? 'warning' : 'error', message: data?.error?.message || `Status ${res.status}`, topUpUrl: 'https://console.groq.com' }
    }
    const data = await res.json().catch(() => ({}))
    const models = (data.data || []).map(m => m.id).slice(0, 5)
    return { id: 'groq', name: 'Groq (LLM Ultra-rápido)', status: 'ok', message: `${(data.data||[]).length} modelos. Latência: ${latency}ms. Free tier.`, models, capabilities: ['Chat'], latencyMs: latency, topUpUrl: 'https://console.groq.com' }
  } catch (err) {
    return { id: 'groq', name: 'Groq (LLM Ultra-rápido)', status: 'error', message: `Erro: ${scrub(err?.message)}`, topUpUrl: 'https://console.groq.com' }
  }
}

// ─── Replicate (Hub de Modelos) ─────────────────────────────────────────
async function checkReplicate() {
  const key = process.env.REPLICATE_API_KEY
  if (!key) return { id: 'replicate', name: 'Replicate (Hub: Vídeo/Imagem)', status: 'unconfigured', message: 'REPLICATE_API_KEY não configurado.', topUpUrl: 'https://replicate.com/account/billing' }
  try {
    const res = await safeFetch('https://api.replicate.com/v1/account', {
      headers: { Authorization: `Bearer ${key}` }
    }, 8000)
    if (!res.ok) return { id: 'replicate', name: 'Replicate (Hub: Vídeo/Imagem)', status: 'error', message: `Status ${res.status}`, topUpUrl: 'https://replicate.com/account/billing' }
    const data = await res.json().catch(() => ({}))
    return {
      id: 'replicate', name: 'Replicate (Hub: Vídeo/Imagem)', status: 'ok',
      message: `Conta: ${data.username || 'ok'}. Hub com Minimax, Stable Video e + modelos.`,
      capabilities: ['Video Generation', 'Image Generation'],
      topUpUrl: 'https://replicate.com/account/billing'
    }
  } catch (err) {
    return { id: 'replicate', name: 'Replicate (Hub: Vídeo/Imagem)', status: 'error', message: `Erro: ${scrub(err?.message)}`, topUpUrl: 'https://replicate.com/account/billing' }
  }
}

// ─── HuggingFace (ZeroGPU + Z-Image-Turbo + Bark) ────────────────────────
async function checkHuggingFace() {
  const key = process.env.HUGGINGFACE_API_KEY
  if (!key) return { id: 'huggingface', name: 'HuggingFace (ZeroGPU + Bark)', status: 'unconfigured', message: 'HUGGINGFACE_API_KEY não configurado.', topUpUrl: 'https://huggingface.co/settings/tokens' }
  try {
    const res = await safeFetch('https://huggingface.co/api/whoami-v2', {
      headers: { Authorization: `Bearer ${key}` }
    }, 8000)
    if (!res.ok) return { id: 'huggingface', name: 'HuggingFace (ZeroGPU + Bark)', status: 'error', message: `Token inválido (status ${res.status})`, topUpUrl: 'https://huggingface.co/settings/tokens' }
    const data = await res.json().catch(() => ({}))
    const isPro = data.isPro || false
    const plan = isPro ? 'PRO' : 'Free'
    return {
      id: 'huggingface', name: 'HuggingFace (ZeroGPU + Bark)',
      status: 'ok',
      message: `Conta: ${data.fullname || data.name} (${plan}). ZeroGPU 40min/dia. Z-Image-Turbo disponível.`,
      capabilities: ['Image Generation (Z-Image-Turbo)', 'Voice (Bark/Coqui)', '110+ LLMs', 'MCP Server'],
      topUpUrl: 'https://huggingface.co/settings/billing'
    }
  } catch (err) {
    return { id: 'huggingface', name: 'HuggingFace (ZeroGPU + Bark)', status: 'error', message: `Erro: ${scrub(err?.message)}`, topUpUrl: 'https://huggingface.co/settings/tokens' }
  }
}
