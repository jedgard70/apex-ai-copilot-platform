...
async function generateWithOpenAI({ safePrompt }) {
  const apiKey = process.env.OPENAI_API_KEY
  if (!apiKey) throw new Error('OPENAI_API_KEY missing for fallback')

  const response = await fetch('https://api.openai.com/v1/images/generations', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
    body: JSON.stringify({ model: 'dall-e-3', prompt: safePrompt, n: 1, size: '1024x1024' })
  })
  ...
  // DALL-E returns a URL. We must fetch it and convert to base64 data URL to match expected output.
...
// ─── Provider: Ideogram Fallback ─────────────────────────────────────────────
async function generateWithIdeogram({ safePrompt }) {
  const apiKey = process.env.IDEOGRAM_API_KEY
  if (!apiKey) throw new Error('IDEOGRAM_API_KEY missing for fallback')

  const response = await fetch('https://api.ideogram.ai/generate', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Api-Key': apiKey },
    body: JSON.stringify({ image_request: { prompt: safePrompt, model: 'V_2', aspect_ratio: 'ASPECT_1_1', magic_prompt_option: 'AUTO' } })
  })

  if (!response.ok) {
    const errorText = await response.text()
    throw new Error(`Ideogram Fallback failed: ${response.status} ${errorText}`)
  }
...
export default async function handler(req, res) {
  ...
  // Removed early return for falKey missing, allow fallback to Gemini to handle it.
  ...
  try {
    ...
    if (falKey) {
      try {
        ...
        lastError = err
        console.error('FAL failed, trying next:', err.message)
      }
    }

    // 2. Secondary Provider: Gemini (Imagen 3)
    if (process.env.GEMINI_API_KEY) {
      try {
        ...
        lastError = err
        console.error('Gemini failed, trying next:', err.message)
      }
    }

    // 3. Tertiary Provider: OpenAI (DALL-E 3)
    if (process.env.OPENAI_API_KEY) {
      try {
        const fallbackResult = await generateWithOpenAI({ safePrompt })
        if (fallbackResult && fallbackResult.images) {
          logUsage(tenantId, 'openai', 'dall-e-3', { units: fallbackResult.images.length }).catch(console.error)
        }
        return sendJson(res, 200, fallbackResult)
      } catch (err) {
        lastError = err
        console.error('OpenAI failed, trying next:', err.message)
      }
    }

    // 4. Quaternary Provider: Ideogram
    if (process.env.IDEOGRAM_API_KEY) {
      try {
        const fallbackResult = await generateWithIdeogram({ safePrompt })
        if (fallbackResult && fallbackResult.images) {
          logUsage(tenantId, 'ideogram', 'ideogram-v2', { units: fallbackResult.images.length }).catch(console.error)
        }
        return sendJson(res, 200, fallbackResult)
      } catch (err) {
        lastError = err
        console.error('Ideogram failed, trying next:', err.message)
      }
    }
    ...
    return sendJson(res, 200, {
      ...
      message: 'No image provider configured. Add FAL_KEY, GEMINI_API_KEY, OPENAI_API_KEY, or IDEOGRAM_API_KEY.',
    })
  } catch (error) {
