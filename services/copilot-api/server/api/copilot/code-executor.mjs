...
export default async function handler(req, res) {
  ...
  if (path === '/api/copilot/code-executor/plan' && req.method === 'POST') {
    try {
      const { buildOwnerCodeExecutionPlan } = await import('../../service/ownerCodeExecutor.mjs')
  ...
  if (path === '/api/copilot/code-executor/validate-command' && req.method === 'POST') {
    try {
      const { validateOwnerCodeCommand } = await import('../../service/ownerCodeExecutor.mjs')
  ...
  if (path === '/api/copilot/code-executor/status' && (req.method === 'POST' || req.method === 'GET')) {
    try {
      const { getOwnerCodeExecutorStatus } = await import('../../service/ownerCodeExecutor.mjs')
  ...
  if (path === '/api/copilot/code-executor/log' && req.method === 'POST') {
    try {
      const { appendExecutionLog } = await import('../../service/ownerCodeExecutor.mjs')
