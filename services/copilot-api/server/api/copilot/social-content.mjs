...
// callOpenAI and callGemini removed
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const resAi = await chatWithFallback({ messages, preferredProvider: 'apex-runtime', maxTokens: 2400 })
    if (resAi.ok) {
      rawText = resAi.data?.choices?.[0]?.message?.content || ''
      providerStatus = resAi.provider || 'fallback'
    } else {
      console.warn('[social-content] AI failed:', resAi.error)
      return sendJson(res, 200, fallback)
    }
  } catch (err) {
    console.warn('[social-content] AI threw error:', err.message)
    return sendJson(res, 200, fallback)
  }
