...
// Removed manual callOpenAI and callGemini
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const resAi = await chatWithFallback({ messages, preferredProvider: 'apex-runtime', maxTokens: 600 })
    if (resAi.ok) {
      ...
      logUsage(tenantId, resAi.provider || 'fallback', resAi.model || 'unknown', { promptTokens: estimatedTokens, completionTokens: 0 }).catch(console.error)
    } else {
      return sendJson(res, 200, {
        refinedScene: scene,
        suggestedCamera: camera,
        suggestedStyle: style,
        aiNote: 'AI provider failed. ' + resAi.error,
      })
    }
