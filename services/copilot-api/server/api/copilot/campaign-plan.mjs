...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (true) { // We let provider router handle the logic now!
      const messages = [
        { role: 'system', content: buildSystemPrompt() },
        { role: 'user', content: buildUserPrompt(goal, campaignGoal, channel, format, audience, offer) }
      ]
      try {
        const resAi = await chatWithFallback({ messages, preferredProvider: 'apex-runtime', maxTokens: 1800 })
        if (resAi.ok) {
          rawText = resAi.data?.choices?.[0]?.message?.content || ''
          providerStatus = resAi.provider || 'fallback'
        } else {
          console.warn('[campaign-plan] AI generation failed, using local plan:', resAi.error)
        }
      } catch (err) {
        console.warn('[campaign-plan] AI generation error, using local plan:', err.message)
      }
    }
