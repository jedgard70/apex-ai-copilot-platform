...
export default async function handler(req, res) {
  ...
  try {
    ...
    const resAi = await chatWithFallback({ messages, preferredProvider: 'apex-runtime', maxTokens: 2048 })
    
    if (resAi.ok) {
      const rawText = resAi.data?.choices?.[0]?.message?.content || ''
      const aiData = parseAiResponse(rawText)
      if (aiData) {
        return sendJson(res, 200, { documents: aiData, providerStatus: resAi.provider || 'fallback' })
      }
    }
