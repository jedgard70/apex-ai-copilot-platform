...
import { collectProductionOperatorStatus } from '../../agent/productionStatus.mjs'
...
async function ensureInteractionsLoaded() {
  ...
  _interactionsPromise = (async () => {
    try {
      const mod = await import('../../providers/gemini-interactions.mjs')
...
import { buildCodeToolDefinitions, executeCodeToolCall, CODE_TOOL_NAMES } from '../../agent/codeTools.mjs'
import { buildGithubToolDefinitions, executeGithubToolCall, GITHUB_TOOL_NAMES } from '../../agent/githubTools.mjs'
import { runLocalWorkerAction } from '../../agent/localWorkerClient.mjs'
import { classifyImageGenRequest, buildImagePrompt, generateImage, buildImageResultReply } from '../../agent/imageGenerationConnector.mjs'
import { classifyVideoGenRequest, generateVideo, buildVideoResultReply } from '../../agent/videoGenerationConnector.mjs'
import { sendAuthkeySms, sendAuthkeyOtp, buildAuthkeyResultReply } from '../../agent/authkeyConnector.mjs'
import { keyRestrictionMiddleware, validateOrigin } from '../../middleware/keyRestriction.mjs'
import { recordAuditEvent } from '../../service/securityAudit.mjs'
...
async function _getRecordCall() {
  ...
  try {
    const mod = await import('../../service/providerAnalytics.mjs')
...
async function executeLiveAgentToolCall(toolCall) {
  ...
  if (name === 'learn_url') {
    ...
    try {
      const { analyzeUrl } = await import('../../service/urlContext.mjs')
...
💡 Tom: profissional, direto, confiante. Markdown rico. Português com inglês técnico. Após entregar, dê próximo passo concreto.`
      ...
      if (geminiApiKey) {
        ...
        try {
          const { chatWithFallback: autoFallback } = await import('../../providers/providerRouter.mjs')
    ...
    if (toolCalls.length) {
      ...
      for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        ...
        if (nativeResult.response.ok) {
          ...
          const { chatWithFallback: toolFallback } = await import('../../providers/providerRouter.mjs')
