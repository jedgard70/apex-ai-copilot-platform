...
Alterar configurações de ambientes ou variáveis sensíveis diretamente no dashboard web da Vercel
Modificar regras de rastreamento de ramificação ou remover/desconectar a Git Integration
...
Executar o build web local para validar integridade: npm run build
Executar o build do aplicativo desktop (.exe): npm run electron:build
Efetuar o deploy automático para Produção (Vercel): git add ., seguido de git commit detalhado e git push origin main.
...
Configuração atual dos ambientes (2026-07-10 — NÃO ALTERAR):

Produção → filial main → domínio www.apexglobalai.com e aplicativo Desktop (.exe)
