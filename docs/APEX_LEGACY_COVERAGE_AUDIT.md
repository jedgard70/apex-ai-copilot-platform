...
| Field Operations / RDO | Legacy field/reporting direction | FieldOps Studio with RDO, photo log, safety, quality, punch list and client report | Partial | No weather connector or inspection approval; NR Compliance now exists as local-first CP11C guidance | Field evidence/source connector |
...
| EVM Analyst Agent | CP11C EVM / Scheduler / NR Panel connected to Budget, Finance, FieldOps and Project Workspace | Implemented local-first | Needs source-connected project controls and confirmed cost/progress feeds | Source-connected EVM controls |
| NR Compliance Agent | CP11C NR checklist, risk matrix and corrective action plan connected to FieldOps and Contracts/Permits | Implemented local-first | Needs current source/legal/safety review connector and qualified approval workflow | NR source/safety review connector |
...
| Scheduler Agent | CP11C Gantt-style task list, milestones, dependencies, lookahead and physical-financial schedule | Implemented local-first | Needs baseline import/export and source-connected schedule integrations | Schedule connector / baseline integration |
...
1. BIM parser/viewer hardening: real IFC evidence extraction and clash metadata.
2. Auth/database/RLS: real client workspace isolation and user roles.
3. Pricing/source connectors: SINAPI upload/source verification and finance/accounting evidence.
4. CP11C source connectors: EVM data feeds, schedule baseline import/export and NR source/safety review.
