...
export function isMetricsIntent(text: string) {
  return /\b(m[eé]tricas|metricas|dashboard de m[eé]tricas|metrics dashboard|observabilidade|health check|endpoint health|module usage|api metrics)\b/i.test(text)
}
