    ...
    <div className="flex items-center gap-4 w-full bg-white/[0.03] rounded-2xl p-4 border border-white/[0.06]">
      <div className="relative shrink-0">
        <div className="p-[2px] rounded-full bg-gradient-to-br from-blue-400 to-violet-500">
          <img src={(user as any).picture || FALLBACK_AVATAR} alt={(user as any).name || "User"} referrerPolicy="no-referrer" className="w-11 h-11 rounded-full object-cover bg-slate-900 block" onError={(e) => { (e.target as HTMLImageElement).src = FALLBACK_AVATAR; }} />
        </div>
        <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-green-400 rounded-full border-2 border-[#060812]" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-white text-sm font-semibold truncate">{(user as any).name}</p>
        <p className="text-slate-400 text-xs truncate mt-0.5">{(user as any).email}</p>
      </div>
      <span className="shrink-0 text-xs px-2 py-0.5 rounded-full bg-green-400/10 text-green-400 border border-green-400/20 font-medium">Active</span>
    </div>
  );
}
