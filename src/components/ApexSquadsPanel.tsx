...
export function ApexSquadsPanel() {
  ...
  const userSquads = squads.filter(s => !s.isSystemModule)
  const systemSquads = squads.filter(s => s.isSystemModule)

  const renderSquadList = (list: any[]) => (
  ...
  return (
    <div className="p-6 bg-slate-900 text-slate-100 min-h-full">
      ...
      <div className="mb-12">
        <h3 className="text-xl font-semibold mb-4 border-b border-slate-700 pb-2">Modules Core Squads (Frota Oficial de Deploy)</h3>
        {systemSquads.length === 0 ? (
          <p className="text-slate-400">Nenhum squad de módulo mapeado.</p>
        ) : (
          renderSquadList(systemSquads)
        )}
      </div>

      <div>
        <h3 className="text-xl font-semibold mb-4 border-b border-slate-700 pb-2">Seus Squads Customizados</h3>
        {userSquads.length === 0 ? (
          <p className="text-slate-400">Nenhum squad customizado em operação.</p>
        ) : (
          renderSquadList(userSquads)
        )}
      </div>
    </div>
  )
