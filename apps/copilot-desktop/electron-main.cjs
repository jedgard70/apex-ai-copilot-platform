...
function getAppRoot() {
  ...
  if (fs.existsSync(path.join(asarRoot, "services", "copilot-api", "server.mjs"))) return asarRoot;

  const unpackedRoot = path.join(process.resourcesPath, "app.asar.unpacked");
  if (fs.existsSync(path.join(unpackedRoot, "services", "copilot-api", "server.mjs"))) return unpackedRoot;

  const resourcesRoot = process.resourcesPath;
  if (fs.existsSync(path.join(resourcesRoot, "services", "copilot-api", "server.mjs"))) return resourcesRoot;
...
app.whenReady().then(async () => {
  ...
  const serverScript = path.join(appRoot, "services", "copilot-api", "server.mjs");
  ...
  if (!fs.existsSync(serverScript)) {
    log(`[Apex] services/copilot-api/server.mjs nao encontrado em ${serverScript}`);
    if (mainWindow) {
      mainWindow.loadURL(startupHtml({
